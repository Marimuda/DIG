...
def create_batch_info(data, edge_counter):
    x, edge_index, batch, batch_size = data.x, data.edge_index, data.batch, data.x.shape[0]
    ...
    for value, m in zip(unique, n_per_graph):
        n_batch[batch == value] = m.float()
